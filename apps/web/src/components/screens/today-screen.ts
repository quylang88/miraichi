...
export function renderTodayScreen(input: {
  ...
  return `<section class="${screenClass('today', activeTabId)}" id="screen-today" data-shell-tab-panel="today" aria-labelledby="today-title">
    ...
    <div class="metric-grid">${metricRow(translate('today.netPnl'), `${net > 0 ? '+' : ''}${net} pts`)}${metricRow(translate('today.openExposure'), `${exposure} pts`)}${metricRow(translate('today.ongoingBets'), String(ongoing.length))}</div>
    ...
    <div class="section-heading"><h2>${escapeHtml(translate('today.ongoingBets'))}</h2></div><div class="stack">${cards || `<p class="empty-state">${escapeHtml(translate('bets.emptyOngoing'))}</p>`}</div>
  </section>`;
}
