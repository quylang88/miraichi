...
function renderSheets(bankroll: BankrollViewState, translate: TranslateFunction): string {
  const accountOptions = bankroll.status === 'ready'
    ? bankroll.accounts.filter((account) => !account.archived).map((account) => `<option value="${escapeHtml(account.accountId)}">${escapeHtml(account.label)}</option>`).join('')
    : '';
  ...
  return `<div class="sheet-backdrop" data-close-sheet></div>
    <section class="sheet" id="add-sheet" aria-hidden="true" aria-modal="true" aria-labelledby="add-sheet-title" role="dialog" data-focus-trap>${header('add-sheet', translate('bets.add'), translate('bets.manualMatch'))}<div class="sheet-body"><form class="form-grid" id="add-form" novalidate>
      ...
      <div class="field"><label for="account-field">${escapeHtml(translate('bets.account'))}</label><select class="field-select" id="account-field" name="account-field"><option value="">${escapeHtml(translate('common.unassigned'))}</option>${accountOptions}</select></div>
    ...
    <section class="sheet" id="transfer-sheet" aria-hidden="true" aria-modal="true" aria-labelledby="transfer-sheet-title" role="dialog" data-focus-trap>${header('transfer-sheet', translate('bankroll.transfer'))}<div class="sheet-body"><form class="form-grid" id="transfer-form"><div class="field"><label for="transfer-target">${escapeHtml(translate('bankroll.toAccount'))}</label><select class="field-select" id="transfer-target" name="toAccountId" required>${accountOptions}</select></div><div class="field"><label for="transfer-amount">${escapeHtml(translate('bankroll.points'))}</label><input class="field-input" id="transfer-amount" name="amountPoints" type="number" min="0.01" step="0.01" required></div><div class="field"><label for="transfer-note">${escapeHtml(translate('bets.note'))}</label><textarea class="field-textarea" id="transfer-note" name="note"></textarea></div><div class="sheet-actions"><button class="secondary-button" type="button" data-close-sheet>${escapeHtml(translate('common.cancel'))}</button><button class="primary-button" type="submit">${escapeHtml(translate('common.confirm'))}</button></div></form></div></section>
    <section class="sheet" id="settings-sheet" aria-hidden="true" aria-modal="true" aria-labelledby="settings-sheet-title" role="dialog" data-focus-trap>${header('settings-sheet', translate('settings.title'))}<div class="sheet-body"><form class="form-grid" id="settings-form"><div class="field"><label for="settings-locale">${escapeHtml(translate('settings.language'))}</label><select class="field-select" id="settings-locale" name="locale"><option value="en">${escapeHtml(translate('settings.english'))}</option><option value="vi">${escapeHtml(translate('settings.vietnamese'))}</option></select></div><div class="field"><label for="settings-density">${escapeHtml(translate('settings.displayDensity'))}</label><select class="field-select" id="settings-density" name="displayDensity"><option value="standard">${escapeHtml(translate('settings.standard'))}</option><option value="compact">${escapeHtml(translate('settings.compact'))}</option></select></div><button class="primary-button" type="submit">${escapeHtml(translate('common.save'))}</button></form></div></section>`;
}
