...
export type SeasonHydrationLeaseResult<T> =
  | { acquired: true; value: T }
  | { acquired: false };

export class SeasonHydrationJobLease {
  ...
  constructor(options: { dataRoot: string; staleAfterMs?: number; now?: () => Date }) {
    ...
    if (!Number.isInteger(this.staleAfterMs) || this.staleAfterMs < 1) {
      throw new Error('Season hydration lease staleAfterMs must be a positive integer.');
    }
  }

  async tryWithLease<T>(operation: () => Promise<T>): Promise<SeasonHydrationLeaseResult<T>> {
