...
export async function syncServingMatchStoreToCloud(options:{servingRepository:{loadSnapshot():Promise<LocalMatchSnapshot>};adapter:CloudPersistenceAdapter;ownerProfileId:string}){
  ...
  for(const match of snapshot.matches){const validation=validateLocalMatch(match);if(!validation.ok)throw new Error(`Serving match store is invalid: ${validation.errors.join('; ')}`);if(match.competition.type!=='national-team'||String(match.status)==='in_play')throw new Error('Serving match store is invalid for Phase 9 cloud sync');}
