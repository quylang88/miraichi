...
export async function settleBet({adapter,ownerProfileId,betId,command,now}:{
  ...
  const event:BetSettlementEvent={...command,ownerProfileId,betId,bankrollAccountId:record.bankrollAccountId,calculatedProfitLossPoints:calculated,ledgerDeltaPoints:ledgerDelta,effectiveAt,occurredAt:now};
  const updated:CloudBetRecord={...record,status:'settled',settlementType:command.settlementType,profitLossPoints:calculated,settledAt:record.settledAt??effectiveAt,postBetPlanAdherence:command.planAdherence,...(command.lessonNote?{postBetLessonNote:command.lessonNote}:{}),updatedAt:now};
