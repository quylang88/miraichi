...
export async function handleBankroll(req:IncomingMessage,res:ServerResponse,deps:CloudRouteDependencies):Promise<void>{
  ...
  try{
    ...
    if(isLedger&&req.method==='POST'){
      if(!text(payload.entryId)||!text(payload.accountId)||!['deposit','withdrawal','correction'].includes(String(payload.entryType))||!finite(payload.amountPoints)||payload.amountPoints===0||!text(payload.occurredAt))return sendError(res,400,'invalid_cloud_record','Ledger payload is invalid.');
    ...
    if(isTransfers&&req.method==='POST'){
      ...
      try{return sendJson(res,201,await deps.adapter.createBankrollTransfer({transferId:text(payload.transferId)!,ownerProfileId:deps.ownerProfileId,fromAccountId:text(payload.fromAccountId)!,toAccountId:text(payload.toAccountId)!,amountPoints:Number(payload.amountPoints),...(text(payload.note)?{note:text(payload.note)!}:{}),occurredAt:text(payload.occurredAt)!}));}catch(error){if(String(error).includes('archived'))return sendError(res,409,'bankroll_account_archived','Archived accounts cannot transfer points.');return sendError(res,404,'bankroll_account_not_found','Bankroll account not found.');}
    }
