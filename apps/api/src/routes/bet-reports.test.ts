...
describe('bet reports route',()=>{
  it('uses a validated owner timezone when numeric discipline thresholds were never configured',async()=>{const adapter=createMemoryCloudPersistenceAdapter();const out=res();await handleBetReports(req('/api/v1/bet-reports?period=week&anchor=2026-08-21&timeZone=Asia%2FTokyo') as never,out as never,{adapter,ownerProfileId:'owner-primary'});expect(out.statusCode).toBe(200);expect(JSON.parse(out.body)).toMatchObject({netProfitLossPoints:0,period:{timeZone:'Asia/Tokyo'}});});
  it('rejects an invalid fallback report timezone',async()=>{const adapter=createMemoryCloudPersistenceAdapter();const out=res();await handleBetReports(req('/api/v1/bet-reports?period=week&anchor=2026-08-21&timeZone=not%2Fa-zone') as never,out as never,{adapter,ownerProfileId:'owner-primary'});expect(out.statusCode).toBe(400);expect(JSON.parse(out.body)).toMatchObject({error:{code:'invalid_report_query'}});});
