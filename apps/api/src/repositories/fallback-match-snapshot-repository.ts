...
const isMissing=(error:unknown)=>(error as {code?:string}).code==='serving_match_store_missing';
export class FallbackMatchSnapshotRepository implements MatchSnapshotRepository {
  ...
  async getStatus(){const local=await this.local.getStatus();if(local.freshness!=='missing')return local;try{const cloud=await this.cloud.getStatus();if(cloud.freshness==='missing')return{...local,warnings:[...local.warnings,'cloud_match_store_unavailable']};return{...cloud,warnings:[...cloud.warnings,'cloud_match_store_fallback']};}catch{return{...local,warnings:[...local.warnings,'cloud_match_store_unavailable']};}}
}
