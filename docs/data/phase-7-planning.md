...
| **Recommendation** | **Primary Provider** | **Secondary/Backup (Fixtures only)** | **Secondary/Backup (Odds only)** |

### 2.2 API-Football Free Tier Characteristics
The free tier of API-Football via RapidAPI is highly suited for the development and offline testing phases of Miraichi:
...
**API-Football** is recommended as the primary data provider for Miraichi. It provides a unified API interface that supplies both fixture metadata (teams, schedules, lineups, live status, final scores) and bookmaker odds (implied odds from multiple bookmakers). Using a single provider reduces the complexity of joining disparate match identifiers, which would be required if merging Football-Data.org fixtures with TheOdds-API odds.
...
The worker reads this configuration at runtime, instructing the API-Football adapter to fetch and parse the data matching `providerLeagueId`. The downstream systems only see `competitionId`.
...
1. **Raw Payload Ingestion**: The raw JSON API response is fetched and immediately written to the local cache.
...
3. **Internal Registry Mapping**: The provider's team IDs and league IDs are translated into internal generic keys (`team-alpha`, `competition-beta`) using a dynamic registry.
...
To ensure strict data lineage and allow offline R&D without re-querying third-party APIs, the ingestion engine implements a raw cache layer:
...
* **Mechanism**: Every HTTP response body returned by API-Football is written to disk alongside metadata (fetch timestamp, status code). When performing feature engineering or running backtests, the pipeline reads from this local directory first. If a file exists, it skips the network request. This guarantees exact reproducibility of training runs.
...
Before a trained model can be promoted to staging or used to generate live suggestions in the PWA, it must satisfy three quantitative verification gates on the out-of-sample Test Set.

```mermaid
graph TD
    Model[Trained Model Candidate] --> Gate1{Gate 1: Brier Score vs Baseline}
    Gate1 -->|BS Improved >= 1%| Gate2{Gate 2: Sample Size}
    Gate1 -->|Failed| Reject[Reject Model]
    Gate2 -->|N >= 100| Gate3{Gate 3: ECE}
    Gate2 -->|Failed| Reject
    Gate3 -->|ECE < 5%| Approve[Approve for Staging]
    Gate3 -->|Failed| Reject
```

### 6.1 Gate 1: Brier Score Improvement
* **Requirement**: The model's Brier Score on the out-of-sample Test Set must be at least 1% better (lower) than the bookmaker implied probability baseline:
...
* **Requirement**: The out-of-sample Test Set must contain a minimum of 100 historical match fixtures.
...
* **Requirement**: The Expected Calibration Error (ECE) on the out-of-sample Test Set must be strictly less than 5%:
  $$ECE < 0.05$$
