...
- For Miraichi plans, new app/package implementation modules are TypeScript-first. Specify `.ts` and `*.test.ts` for new `apps/*/src` or `packages/*/src` components, services, config, validators, and pure helpers unless the file is an explicit legacy/runtime bridge with a documented reason.
