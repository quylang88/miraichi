...
describe('provider-neutral ingestion contracts', () => {
  ...
  it('validates api-football raw payload envelopes and source metadata', () => {
    expect(validateRawProviderPayloadEnvelope({
      schemaVersion: 'miraichi.provider.raw.v1',
      provider: 'api-football',
      endpointKey: 'fixtures.league.39.2026',
      urlPath: '/fixtures',
      ...
      source: {
        allowlistEntryId: 'api-football-eng-premier-league',
        leagueId: 39,
        season: 2026,
        queryType: 'season'
      },
    ...
    expect(validateRawProviderPayloadEnvelope({
      ...
      errors: expect.arrayContaining([
        expect.stringContaining('allowlistEntryId'),
        expect.stringContaining('leagueId'),
        expect.stringContaining('season')
      ])
    });
  });

  it('links canonical matches to api-football fixtures and tracks field provenance', () => {
    expect(validateProviderLink({
      entityType: 'match',
      entityId: 'match-eng-premier-league-2026-1001',
      provider: 'api-football',
      providerEntityType: 'fixture',
