...
import { syncServingMatchStoreToCloud } from '../apps/api/src/services/cloud-match-snapshot-sync.js';
import type { LocalMatchSnapshot } from '../apps/api/src/repositories/serving-match-store.js';

export async function runServingMatchStoreCloudSync(options:{servingRepository:{loadSnapshot():Promise<LocalMatchSnapshot>};adapter:CloudPersistenceAdapter;ownerProfileId:string;log?:(message:string)=>void}){
  const result=await syncServingMatchStoreToCloud(options);(options.log??console.log)(`Synced serving match store ${result.snapshotId}: ${result.matchCount} matches to ${result.provider}`);return result;
}
...
async function main(){loadEnv();const config=readCloudPersistenceConfig();await runServingMatchStoreCloudSync({servingRepository:new ServingMatchStoreRepository(),adapter:createCloudPersistenceAdapter(config),ownerProfileId:config.ownerProfileId});}
